"""Ferrari PowerPoint Template — build entrypoint.

Run:   uv run python build.py
Output: out/Ferrari-Template.pptx
"""
...
from master import (
    apply_ferrari_theme, set_slide_size, ensure_n_layouts,
...
OUT_DIR = HERE / "out"
OUT_FILE = OUT_DIR / "Ferrari-Template.pptx"
...
def main(include_demo_slides: bool = True):
    ...
    # 2. Theme (colour + font scheme) → Ferrari tokens
    apply_ferrari_theme(prs)

    # 3. Grow the layout set to hold every Ferrari layout
    ensure_n_layouts(prs, len(layouts.LAYOUTS))

    # 4. Strip stock shapes from master + every layout, paint Ferrari chrome/content
