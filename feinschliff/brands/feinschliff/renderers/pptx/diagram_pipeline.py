...
def _strip_theme_bsh(dsl: str) -> str:
    ...
    We always apply the feinschliff theme ourselves via apply_feinschliff_theme,
...
def render_diagram(dsl: str, slug: str, out_dir: Path) -> tuple[Path, list[str]]:
    ...
    dsl: source DSL text. Any leading `theme bsh` line is stripped — we
         always feinschliff-theme the output via apply_feinschliff_theme here.
    ...
    import apply_feinschliff_theme  # from feinschliff/brands/feinschliff/renderers/excalidraw
    ...
    themed_doc = apply_feinschliff_theme.apply_feinschliff(copy.deepcopy(generic_doc))
