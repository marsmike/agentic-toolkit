...
Corner geometry is **policy-driven**: each component reads its radius slot
from `tokens.json` (`radius.btn`, `radius.chip`) and dispatches via
`add_rounded_rect`. BMW sets these to 0px → `add_rounded_rect` falls
through to `add_rect`, rendering the exact sharp rectangle we always
shipped. Spotify will set them to 9999 → fully-rounded pills, no
