...
def add_rounded_rect(
    ...
    *,
    ...
    """Add a rectangle whose corner radius is read from a brand-pack token.

    `radius_px=0` falls through to `add_rect` so BMW (and any other brand
    with `radius.btn`/`radius.chip`/`radius.card == 0`) renders the exact
    same `MSO_SHAPE.RECTANGLE` it always did — visually identical, byte-
    identical OOXML.

    `radius_px > 0` renders `MSO_SHAPE.ROUNDED_RECTANGLE` with the corner
    adjustment computed against the shape's smaller edge. `radius_px` is
    clamped to half the smaller edge — so a `9999` value becomes a full
    pill (Spotify's primary CTA shape) and `8` on a 200×120 card becomes
    `8 / (120/2) = 0.133` — a soft-cornered card plate.

    This is the single dispatch point for brand-specific corner geometry.
    Components (button, chip, card) call this directly with the token
    they care about; brand packs change shape behavior by editing
    `tokens.json`, never the renderer.
    """
    if radius_px <= 0:
        return add_rect(
