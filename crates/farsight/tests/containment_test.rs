...
fn scratch(tag: &str) -> PathBuf {
    let root = std::env::temp_dir().join(format!("farsight-containment-{tag}-{}", std::process::id()));
...
fn links_out_of_the_vault_are_not_notes() {
    ...
    let names = farsight::discover_notes(&vault).iter().map(|p| p.file_name().unwrap().to_string_lossy().to_string()).collect::<Vec<_>>();
