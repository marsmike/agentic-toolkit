...
/// (a) Indexing recovers the expected node count — one per `vault/Index.md` entry: the
/// active-content notes plus the root-level note (`Alex-Vega.md`) whose own frontmatter declares
...
fn index_recovers_expected_node_count_and_density() {
    ...
    // active note, the root persona note (status: active) included since Index.md
    // is generated by index_build.py (review-01 GLM-1). Derived, not hardcoded, so
    // vault growth (e.g. new concept notes) is not a test-breaking event [earned:
    // docs-audit R6 2026-07-26 — hardcoded 73 broke when the vault grew to 79].
    ...
    assert_eq!(
        ...
        "expected one node per Index.md entry"
    );
