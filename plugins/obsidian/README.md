...
| `radar_ledger.py` | nothing: reads the radar's ledgers (`00_Memory/radar/state.jsonl`, `promoted.jsonl`) for the dashboard, the report and the watchdog — worth-or-strong items with their fate, per-interest counts named from the interests note, strong items per ISO week, rising interests |
| `watchdog.py` | nothing: prints whether the pipeline is alive and well from the vault alone (last `pipeline …` commit's age and summary, a stale lock, parked captures, a backlog, a DLQ note less than a day old), exit 1 on a problem. The watchdog routine runs it between pipeline runs and notifies only then |
...
| `dashboard_build`, `report_build`, `watchdog` (radar, R12) | From fixture ledgers: worth-or-strong items with promoted / in-vault fate, per-interest counts named from the interests note, today's counts, a `javascript:` url never a link; the watchdog's `facts.week` and its Sunday-only `weekly_digest` |
| `watchdog` (R12) | In a git sandbox: a run an hour old is ok; a 4-hour gap is `stale`; "1 failed" or "build failed" in the summary is `failed` and a clean run clears it; a 7-hour lock is `hung`; parked captures, more than a batch waiting and a DLQ note from today each fire; an older open DLQ note is listed, not alerted; the CLI exits 1 / 0 |
